...
# fbpad launch script, "created" just by paring off swaths of koreader.
# It's probably still doing way more than necessary.
...
if [ "${VIA_NICKEL}" = "true" ]; then
    ...
    killall -q -TERM nickel hindenburg sickel fickel strickel fontickel adobehost foxitpdf iink dhcpcd-dbus dhcpcd bluealsa fmon nanoclock.lua
...
export FBPAD_INSTALL_PREFIX="/mnt/onboard/.adds/fbpad"
export PATH="$FBPAD_INSTALL_PREFIX/bin:$PATH"
export LD_LIBRARY_PATH="$FBPAD_INSTALL_PREFIX/lib:$LD_LIBRARY_PATH"
