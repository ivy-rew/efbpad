...
if not os.path.isfile('keymap.pickle'):
    the_keymap = [['']*n_maps for _ in range(n_keys)]
    for ikey, key in enumerate(dict_keys.keys()): # For each key...
...
print('/* Autogenerated by keymap_dump.py */')

# Key #defines
for ikey, key in enumerate(dict_keys.keys()):
    print(f'#define {key} {dict_keys[key]}')

# Keymap literals
print(f'''
const char *keymap[{n_keys}][{n_maps}] = {{\n''', end='')
for ikey, key in enumerate(dict_keys.keys()):
    print(f'    {{', end='')
